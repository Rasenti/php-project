<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Accueil</title>
    <link rel="stylesheet" href="/projet/assets/style.css" />
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM" crossorigin="anonymous">
</head>

<body>

    <header class="container-fluid p-0">

        <div class="navbar p-0">
            <div class="logo_container col-lg-2 col-6 m-0">
                <img class="logo img-fluid" src="uploads/logo.svg" alt="Logo">
            </div>
            <ul class="navitems d-flex col-lg-4">
                <li class="btn_navbar"><a href="">Encyclopédie</a></li>
                <li class="btn_navbar"><a href="">Générateur</a></li>
                <li class="btn_navbar"><a href="">Administration</a></li>
            </ul>
            <div class="button_container col-lg-2 col-6 m-0">
                <button class="connect"><img src="/projet/uploads/connection.png" alt="Icon de connexion"></button>
            </div>
        </div>

    </header>